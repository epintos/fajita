...
public class AllUsesTransformation extends FajitaSourceTransformation {
	...
	public ProblemReport execute() {
		...
		return execute(
			new FajitaSourceTransformVisitor<AllUsesTransformation>(
				configuration, this)
		);
	}
...
//					/**/
		...
		protected void visitVariableDeclaration(VariableDeclaration x) {
			List<? extends VariableSpecification> v = x.getVariables();
			for(VariableSpecification variable: v) {
//				System.out.println(variable.getIdentifier().getText() + " - ");
			}
...
//			System.out.println(x.getName());
			super.visitVariableSpecification(x);
		}
		
		@Override
		public void visitMinusAssignment(MinusAssignment x) {
			System.out.println(x.getArity() + " - " + x.getArguments().toString());
