...
public class MultipleConditionTransformation extends FajitaSourceTransformation {
	 ...
	 * 
	 */
	public MultipleConditionTransformation(FajitaConfiguration configuration,
	...
	public ProblemReport execute() {
		...
		return execute(new FajitaSourceTransformVisitor<MultipleConditionTransformation>(
				configuration, this));
	}
	...
	private static class FajitaBranchDiscoveryVisitor extends SourceVisitor {
		...
		public FajitaBranchDiscoveryVisitor(FajitaConfiguration configuration,
				MultipleConditionTransformation transformation,
				...
				for (Statement st : list) {
					if (isAssignment(st)) {
						...
						recursiveExpressionExplorer(
								((Return) st).getExpression(), expressions, true);
					}
		...
		private void recursiveExpressionExplorer(Expression ex,
				List<Expression> expressions, boolean isReturn) {
			if (ex instanceof LogicalAnd) {
				...
				Equals n = new Equals(new ParenthesizedExpression(ex),
						new BooleanLiteral());
				expressions.add(n);
				expressions.add(ex);
