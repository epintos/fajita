...
public abstract class FajitaSourceTransformation extends Transformation {
     ...
     * 
     */
    public FajitaSourceTransformation(FajitaConfiguration configuration, CrossReferenceServiceConfiguration recoder,
    ...
    public static class FajitaSourceTransformVisitor<T extends FajitaSourceTransformation> extends SourceVisitor {
         ...
         * 
         */
        public FajitaSourceTransformVisitor(FajitaConfiguration configuration, T transformation) {
            this.configuration = configuration;
        ...
        public void visitAnnotationUse(AnnotationUseSpecification a) {
            String roopsAnnotations = "(" + ROOPS_ANNOTATION_NROFGOALS + "|" + ROOPS_ANNOTATION_BENCHMARK_METHOD + "|"
        ...
        public void visitMethodDeclaration(MethodDeclaration x) {
            ...
            if (x.getName().equals(configuration.getMethodToCheck())
                ...
                Identifier goalInitializationId = programFactory.createIdentifier(FAJITA_INITIALIZATION_METHOD);
                MethodReference goalInitialization = programFactory.createMethodReference(goalInitializationId);
                ASTList<Statement> newBody = new ASTArrayList<Statement>();
                newBody.add(goalInitialization);
                newBody.addAll(x.getBody().getBody());
                StatementBlock newStatementBlock = programFactory.createStatementBlock(newBody);
            ...
            if (fullName.equals(configuration.getClassToCheck())) {
                ...
                for (int i = 0; i < configuration.getDiscoveredGoals(); i++) {
                    if (configuration.getCoveredGoals().contains(i))
                        continue;
                    Identifier fajitaGoalId = programFactory.createIdentifier(FAJITA_GOAL_TAG + "_" + i);
                    ...
                    FieldDeclaration fieldDeclaration = programFactory.createFieldDeclaration(initializationSpecifiers, booleanReference,
                            fajitaGoalId, null);
                ...
                // programFactory.createIdentifier(FAJITA_LOOP_EXHAUSTION_TAG);
                // / Identifier booleanId =
                // programFactory.createIdentifier("boolean");
                // / TypeReference booleanReference =
                // programFactory.createTypeReference(booleanId);
                // / FieldDeclaration fieldDeclaration =
                // programFactory.createFieldDeclaration(
                // / initializationSpecifiers, booleanReference, fajitaGoalId,
                // null);
                // / x.getMembers().add(fieldDeclaration);
                // / }

                // ADD SPECIAL ROOPS ARRAY DECLARATION SO TACO DOES NOT DETECT
                // IT AS UNUSED
                if (importsRoopsArray) {
                    ...
                    FieldDeclaration fieldDeclaration = programFactory.createFieldDeclaration(initializationSpecifiers, roosArrayReference,
                            roopsArrayVar, null);
                    x.getMembers().add(fieldDeclaration);
                }
                ...
                if (configuration.getCoverageCriteria() == CoverageCriteria.ALL_DEF_USES) {
                    addVariablesDeclarations(x, configuration.getAllUsesAuxVariables());
                    initializationBody.addAll(buildAuxVariablesIntialization(configuration.getAllUsesAuxVariables()));
                }
                ...
                // USE THE SPECIAL ROOPS ARRAY INSTANCE SO TACO DOES NOT DETECT
                // IT AS UNUSED
                if (importsRoopsArray) {
                    Identifier roopsArrayVar = programFactory.createIdentifier("myRoopsArray");
                    UncollatedReferenceQualifier fajitaGoal = programFactory.createUncollatedReferenceQualifier(roopsArrayVar);
                    CopyAssignment copyAssignment = programFactory.createCopyAssignment(fajitaGoal, programFactory.createNullLiteral());
                    initializationBody.add(copyAssignment);
                }

                StatementBlock initializationStatementBlock = programFactory.createStatementBlock(initializationBody);

                // CREATE A GOAL INITIALIZATION METHOD
                Identifier initializationId = programFactory.createIdentifier(FAJITA_INITIALIZATION_METHOD);
                ...
                MethodDeclaration initializaionDeclaration = programFactory.createMethodDeclaration(initializationSpecifiers,
                        voidReference, initializationId, initializationParameters, initializationThrows);
        ...
        private void addVariablesDeclarations(ClassDeclaration x, List<String> allUsesAuxVariables) {
            for (String variable : allUsesAuxVariables) {
                ...
                FieldDeclaration fieldDeclaration = factory.createFieldDeclaration(initializationSpecifiers, booleanReference,
                        fajitaGoalId, null);
                x.getMembers().add(fieldDeclaration);
            }
        }

        private Collection<? extends Statement> buildAuxVariablesIntialization(List<String> allUsesAuxVariables) {
            List<Statement> retList = new LinkedList<>();
            for (String variable : allUsesAuxVariables) {
                ProgramFactory factory = transformation.getProgramFactory();
                Identifier fajitaGoalId = factory.createIdentifier(variable);
                UncollatedReferenceQualifier fajitaGoal = factory.createUncollatedReferenceQualifier(fajitaGoalId);
                BooleanLiteral reachedLiteral = factory.createBooleanLiteral(false);
        ...
        public void visitNegative(Negative x) {
            ProgramFactory programFactory = transformation.getProgramFactory();
            Minus minus = programFactory.createMinus(programFactory.createIntLiteral(0), x.getExpressionAt(0));
