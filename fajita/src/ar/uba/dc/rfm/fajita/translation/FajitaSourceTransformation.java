	...
	public static class FajitaSourceTransformVisitor<T extends FajitaSourceTransformation> extends SourceVisitor {
		...
		public void visitMethodDeclaration(MethodDeclaration x) {
			...
			if (splitClassToCheck.length > 1){
				class_to_check += splitClassToCheck[splitClassToCheck.length - 2] + "Instrumented.";
			}
			class_to_check += splitClassToCheck[splitClassToCheck.length - 1];

			if (x.getName().equals(configuration.getMethodToCheck()) &&
					class_to_check.equals(packageName + getContainingClass(x)) &&
			...
			if (x.getName().startsWith("fin")) {
				// IGNORE FINITIZATION
				//					Return repOKReturn = programFactory.createReturn(new NullLiteral());
				//					ASTList<Statement> block = new ASTArrayList<Statement>();
				//					block.add(repOKReturn);
				//					StatementBlock statementBlock = programFactory.createStatementBlock(block);
				//					transformation.replace(x.getBody(), statementBlock);
		...
		//			if (this.getClass().getName().contains("FajitaJavaCodeKoratRemoverTranslator")){
		//
		//
		//				ASTList<Import> imps = cu.getImports();
		//				ASTList<Import> newImps = new ASTArrayList<Import>(); 
		//				for (Import impp : imps){
		//					if (!impp.toSource().contains("korat") && !impp.toSource().contains("Serializable") 
		//							&& !impp.toSource().contains("KoritaConfig") && !impp.toSource().contains("java.util")){
		//						newImps.add(impp);
		//					}
		//				}
		...
		//						}
		//
		//						for (int idx = 0; idx < ((ClassDeclaration)pe).getChildCount(); idx++){
		//							ProgramElement fieldCandidate = ((ClassDeclaration) pe).getChildAt(idx);
		//							if (fieldCandidate instanceof FieldDeclaration){
		//								ASTList<FieldSpecification> fieldSpecs = ((FieldDeclaration)fieldCandidate).getFieldSpecifications();
		...
		public void visitClassDeclaration(ClassDeclaration x) {
			...
			if (!this.getClass().getName().contains("FajitaJavaCodeGoalPrinterTranslator")/* &&
