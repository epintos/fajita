 ...
 * for the boolean query method coverage criteria.
 * 
 * @see FajitaOutputProcessor
 * 
 */
public class ClassOutputProcessor extends FajitaOutputProcessor {

	
	/**
	 * Constructor for a <code>ClassOutputProcessor</code>.
	 * 
	 * @param configuration a <code>FajitaConfiguration</code> with enough
	 *	data to check which goals were covered.
	 *
	 */
	public ClassOutputProcessor(FajitaConfiguration configuration) {
		super(configuration);
	}
	 ...
	 * information about the covered clases.
	 * 
	 */
	@Override
	public void getCoverage() {
		try {
			...
			if (handler.bq_coverage.isEmpty()) {
				System.out.println("No class was covered.");
				System.exit(-2);
			} else {
				for (String goal : handler.bq_coverage.keySet()) {
					...
					if (handler.bq_coverage.get(goal)) {
						if (configuration.getCoveredGoals().add(goalIndex) &&
							...
							System.out.println(configuration.getPartitionPerGoal(goalIndex) + " : COVERED");
					}
				}
			}
		} catch (Exception e) {
