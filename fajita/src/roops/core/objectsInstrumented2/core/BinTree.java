...
public class BinTree {
	...
	static public void addTest(BinTree tree, int x) {
	...
	@roops.util.BenchmarkMethod
	static public void containsTest(BinTree tree, int x) {
		if (tree != null && tree.repOK()) {
			tree.contains(x);
		}
	}
	...
	@roops.util.BenchmarkMethod
	static public void removeTest(BinTree tree, BinTreeNode z) {
	...
	private BinTreeNode treeSuccessor(final BinTreeNode x_param) {
		...
		if (x.right != null) {
			...
			while (y != null && x == y.right) {
				{
					roops.util.Goals
							.reached(13, roops.util.Verdict.UNREACHABLE);
				}
				x = y;
				y = y.parent;
			}
