...
public class DynalloyXlatorVisitor extends DynalloyVisitor {
	...
	private final HashMap<String, AlloyTyping> varsAndTheirTypesComingFromArithmeticConstraintsInContractsByProgram;
	
	private final HashMap<String, List<AlloyFormula>> predsComingFromArithmeticConstraintsInContractsByProgram;
	
	private final HashMap<String, AlloyTyping> varsAndTheirTypesComingFromArithmeticConstraintsInObjectInvariantsByModule;
	
	private final HashMap<String, List<AlloyFormula>> predsComingFromArithmeticConstraintsInObjectInvariantsByModule;
...
//			AlloyTyping at = qfSig.getFields().merge(this.vars);
//			qfSig = new AlloySig(qfSig.isAbstract(), qfSig.isOne(), qfSig.getSignatureId(), at, qfSig.getExtSigId());
//			int factIndex = 0;
//			for (AlloyFormula af : preds){
//				QuantifierRemoverResult qrFact = removeFactQuantifiers(new AlloyAssertion("ArithmeticFact_"+factIndex,this.vars,af));
//				qfResult.qfFacts.add(new AlloyFact(qrFact.qfAssertion.getFormula()));
//				factIndex++;
//			}
//			
...
//		return alloyAST;
	...
	public Object visit(AssertionDeclaration n) {
		...
		for (AlloyFormula af : predsToAddThatComeFromInvariant){
			AlloyFormula fct = addIdxsToPred((PredicateFormula) af);
...
//		AlloyFormula post = addIdxsToPost(n.getPost(), varCollector.collect(new AndFormula(aux_af, program.getFormula())));
		AlloyFormula post = addIdxsToPost(n.getPost(), varCollector.collect(aux_af));		
		
		
//		ImpliesFormula formula = new ImpliesFormula(new AndFormula(aux_af, program.getFormula()), post);
		ImpliesFormula formula = new ImpliesFormula(aux_af, post);		
