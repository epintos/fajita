...
final class JDynAlloyXlatorVisitor extends JDynAlloyVisitor {
	...
	public Object visit(JDynAlloyModule node) {
		...
		String nodeFields = "";
		String modId = node.getModuleId();
//		if (modId.equals("java_lang_ObjectArray") || modId.equals("java_lang_IntArray")){
//			for (int i=0; i<((Vector<String>)children.fields_result).size(); i++) {
//				Object fieldVectorAsObject = ((Vector<Object>)children.fields_result).get(i); 
//				Vector<Object> field = (Vector<Object>)fieldVectorAsObject;
//				String fieldName = ((AlloyVariable) field.get(0)).toString();
//				String fieldType = ((JType) field.get(1)).toString();
//				int posFirstArrow = fieldType.indexOf("->");
//				String actualFieldType = "";
//				if (posFirstArrow >= 0) {
//					actualFieldType = fieldType.substring(posFirstArrow+2);
//				} else {
//					IndexOutOfBoundsException e = new IndexOutOfBoundsException();
//					throw e;
//				}
//				nodeFields = nodeFields + fieldName + " : " + actualFieldType;
//				if (i<((Vector<String>)children.fields_result).size() - 1){
...
//		String signature = origSig.replaceFirst("\\{\\}", "{\n" + nodeFields + "}");
		...
		return new DynalloyModule(node.getModuleId(), null, sb.toString(), 
				...
				assertions, node.getVarsEncodingValueOfArithmeticOperationsInObjectInvariants(), node.getPredsEncodingValueOfArithmeticOperationsInObjectInvariants());
	}
